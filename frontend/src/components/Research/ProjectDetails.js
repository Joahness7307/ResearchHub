...
const ProjectDetails = () => {
  ...
  const [replyInputs, setReplyInputs] = useState({}); // For reply fields
  ...
  const submitRevisionReason = async (reason) => {
    ...
    try {
      ...
      } else if (user.role === "head_admin" || user.role === "admin") {
      ...
      if (user.role === "research_adviser") navigate("/adviser");
  ...
  return (
    <div className="project-details-page">
      <div className="research-details-container">
        ...
        {user && user.role === "research_adviser" && project.status === "pending" && (
        ...
        {user && user.role === "head_admin" && project.status === "endorsed" && (
          <div className="adviser-actions">
            <button
              onClick={async () => {
                setActionLoading(true);
